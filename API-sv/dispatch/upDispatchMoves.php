    ...
    try {
      ...
      if (is_array($result) && !empty($result)) {
        ...
        foreach ($result as $key => $value) {
          ...
          $value['texto'] = htmlspecialchars(strip_tags($value['texto']));
          $value['texto'] = utf8_encode($value['texto']);
          // formateo titulo, para limpiar codigo html y paso a utf-8
          $value['titulo'] = htmlspecialchars(strip_tags($value['titulo']));
          $value['titulo'] = utf8_encode($value['titulo']);
          // formateo despacho, para limpiar codigo html y paso a utf-8
          $value['despacho'] = htmlspecialchars(strip_tags($value['despacho']));
          $value['despacho'] = utf8_encode($value['despacho']);

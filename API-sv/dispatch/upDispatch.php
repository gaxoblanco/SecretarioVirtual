  ...
  public function upDispatch()
  {
    try {
      ...
      if (
        ...
        // formateo la caratula
        $this->exist['caratula'] = htmlspecialchars(strip_tags($this->exist['caratula']));
        // Limpiar saltos de línea y otros caracteres especiales
        $search = array("\r\n", "\n", "\r", "\t");
        $replace = ' '; // Reemplazar por un espacio en blanco
        $this->exist['caratula'] = str_replace($search, $replace, $this->exist['caratula']);
        // Decodificar las entidades HTML
        $this->exist['caratula'] = html_entity_decode($this->exist['caratula']);
        // preparo la consulta

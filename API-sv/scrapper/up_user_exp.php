...
class up_user_exp
{
  ...
  public function getExpedient()
  {
    ...
    foreach ($this->users as &$user) {
      foreach ($user['expedients'] as &$expedient) {
        ...
        $idExpediente = $this->getIdExpediente($expedient['numero_exp'], $expedient['anio_exp']);
  ...
  public function getIdExpediente($numero_exp, $anio_exp)
  {
    try {
      $query = $this->conexion->prepare('SELECT id_expediente FROM expedientes WHERE numero_expediente = :numero_exp AND anio_expediente = :anio_exp');
